...
export default function Projects() {
  ...
  async function load() {
    setLoading(true);
    try {
      setProjects(await req('GET', '/api/projects'));
    } catch (e) {
  ...
  return (
    <div>
      ...
      <div className="tbl-wrap">
        <table>
          <thead><tr><th>Name</th><th>Description</th><th>Created By</th><th>Created At</th><th></th></tr></thead>
          <tbody>
            {loading ? (
              <tr className="empty-row"><td colSpan={5}><span className="spin" /></td></tr>
            ) : projects.length === 0 ? (
              <tr className="empty-row"><td colSpan={5}>No projects found</td></tr>
            ) : projects.map(p => (
              <tr key={p._id}>
                <td><b>{p.name}</b></td>
                <td style={{ color: 'var(--muted)' }}>{p.description || '—'}</td>
                <td style={{ fontSize: 12, color: 'var(--muted)' }}>{shortId(p.createdBy)}</td>
